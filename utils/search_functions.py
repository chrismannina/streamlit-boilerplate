...
def add_to_past_searches(mrn: str, search_options: List[str], query: str, results: List[Dict]):
    ...
    st.session_state.app_state.past_searches = st.session_state.app_state.past_searches[:10]  # Keep only the 10 most recent searches

def delete_selected_searches(selected_indices):
    for index in sorted(selected_indices, reverse=True):
        del st.session_state.app_state.past_searches[index]
    if st.session_state.app_state.search_done and not st.session_state.app_state.past_searches:
